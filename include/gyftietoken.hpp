...
CONTRACT gyftietoken : public contract
{
  ...
  private:
    ...
    {
        profile_table p_t (get_self(), get_self().value);
        auto rank_index = p_t.get_index<"byrank"_n>();
    ...
    uint64_t get_votes_from_rank (const name account, const uint64_t rank)
    {
        ...
        for (auto voter : votes) {
            if (profile_t.get(voter.value).rank == rank) {
                vote_count++;
            }
        }
    ...
    std::set<int> get_voting_ranks (const name account)
    {
        ...
        for (auto voter : votes) {
            rank_set.insert (profile_t.get(voter.value).rank);
        }
        return rank_set;
    }
    ...
    std::set<int> get_proposal_voting_ranks (const uint64_t proposal_id)
    {
        ...
        std::set<int> rank_set;
        for (auto voter : votes) {
            if (profile_t.get(voter.value).rank > 0) {
                rank_set.insert (profile_t.get(voter.value).rank);
            }
        }
        return rank_set;
